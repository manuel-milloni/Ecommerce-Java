		...
		if (menu.equals("carrito")) {
			...
			case "carrito": {
				...
				if (carrito_lista != null) {
					...
					request.setAttribute("carritoProducto", carritoProducto);
			...
			case "incrementar": {
						...
						if (id == c.getIdProducto()) {
							...
							if (c.getStock() < c.getCantidad() + 1) {
								request.setAttribute("mensaje",
										"La cant pedida excede el stock disponible. Stock: " + c.getStock());
								request.getRequestDispatcher("ControladorCarrito?menu=carrito&accion=carrito")
				...
				request.getRequestDispatcher("ControladorCarrito?menu=carrito&accion=carrito").forward(request,
						response);
				break;
